...
interface RequestWithUser extends Request {
  userId?: string
  userEmail?: string
  userRole?: UserRoles | RestaurantStaffMemberRole
  accessToken?: string
  refreshToken?: string
}
...
export class AuthGuard implements CanActivate {
  ...
  private async handleTokenRefresh(request: RequestWithUser, response: any, refreshToken: string, idToken: string): Promise<boolean> {
  ...
  private async processTokens(request: RequestWithUser, accessToken: string, idToken: string): Promise<boolean> {
    ...
    if (user) {
      // Si es un usuario normal, usar sus datos
      request.userRole = user.role
      request.userId = user.id
      request.userEmail = (decodedToken as { email: string }).email
      request.accessToken = accessToken
    } else {
      // Si no se encuentra en usersService, buscar en restaurantsService
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      const staff = await this.restaurantsService.findStaffByIdCognito(decodedToken.sub.toString())

      if (staff) {
        // Si es un miembro del staff, usar sus datos
        request.userRole = staff.role
        request.userId = staff.id
        request.userEmail = (decodedToken as { email: string }).email
        request.accessToken = accessToken
      } else {
        // Si no se encuentra en ningún servicio, lanzar error
